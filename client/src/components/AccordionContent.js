...
const AccordionContent = ({filters,showContent}) => {
    ...
    const renderFilters=filters.map((filter)=>{
        return <p className='accordion-content' onClick={()=>dispatch(doFilter(filter))}>{filter}</p>
    })

    return (
        <div ref={content} style={{maxHeight: `${height}`}} className="accordion-content-wrapper">
            {renderFilters}
        </div>
    )
}
