...
const fabricFilters= ['Orgazana','Satin','Silk','Velvet','Banarasi Silk','Cotton','Lycra']
const sizeFilters= ["US 2","US 4","US 4","US 6","US 8","US 10","US 12","US 14"]
const priceFilters= [5000,25000,45000,65000,85000,105000]
const styleFilters= ['Heavy Blouse','Fish Cut','Traditional','Designer','Asymmetric','Jacket Style','Printed']
const workFilters= ['Cut beads','Abstract Print','Handwork','Geometric Print','Floral Print','Lace']
...
const Filter = () => {
    ...
    return (
        <div className="filter">
            ...
            {filteredItems && (filteredItems.length< productList.length ? <button onClick={()=>dispatch(clearFilters())}>Clear Filters</button> : '')}
            </div>
        </div>
    )
}
